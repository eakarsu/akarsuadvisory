...
export default function AdminContacts() {
  ...
  const generateAndSend = async (item) => {
    ...
    try {
      const res = await api('/api/admin/ai/reply-contact', {
        method: 'POST',
        body: JSON.stringify({
          name: item.name,
          email: item.email,
  ...
  const scoreThisLead = async (item) => {
    try {
      await api('/api/admin/ai/score-lead', {
        method: 'POST',
        body: JSON.stringify({ type: 'contact', id: item.id, name: item.name, company: item.company, subject: item.subject, message: item.message }),
      });
      load();
    ...
    <div className="admin-page">
      ...
      <div className="admin-table-wrap">
        <table className="admin-table">
          <thead><tr><th>Name</th><th>Email</th><th>Company</th><th>Subject</th><th>Date</th><th>Score</th><th>Status</th><th>Actions</th></tr></thead>
          <tbody>
            {filtered.map(i => (
            ...
            {filtered.length === 0 && <tr><td colSpan={8} style={{ textAlign: 'center', padding: 40, color: '#94a3b8' }}>No contacts found.</td></tr>}
          </tbody>
        </table>
      </div>
              ...
              {aiReply ? (
                <div style={{ padding: 16, background: '#f7f7f7', borderRadius: 8, border: '1px solid #99f6e4' }}>
                  ...
                  <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
                    ...
                    <a href={`mailto:${selected.email}?subject=Re: ${selected.subject || 'Your inquiry'} - Akarsu Advisory&body=${encodeURIComponent(aiReply)}`} className="btn btn-primary" style={{ padding: '6px 14px', fontSize: 12, textDecoration: 'none' }}>Open in Email Client</a>
                  </div>
                </div>
              ) : (
